...
function howToInstall(pageUrl) {
  return (
    ...
    `<ol>` +
    `<li>Завантаж і встанови лаунчер із <a href="${SITE}">lbklauncher.com</a>.</li>` +
    `<li>Знайди гру в лаунчері та встанови локалізацію через нього.</li>` +
    `</ol>` +
...
async function main() {
  ...
  const file = { name: SOURCE_NAME, localizations };
