...
function safeVariantsForApplist(title) {
  ...
  // Match order matters (longer forms before shorter) so "xviii" isn't cut to "xv" by the greedy engine.
  const romToAr = (t) => t.replace(/\b(xviii|xvii|xvi|xiv|xix|xx|xv|xii|xiii|xi|x|ix|viii|vii|vi|iv|v|iii|ii|i)\b/gi, (m) => {
  ...
  const arToRom = (t) => t.replace(/\b(\d{1,2})\b/g, (m) => {
    ...
    const n = Number(m); return n >= 1 && n <= 20 ? R[n] : m;
  });
